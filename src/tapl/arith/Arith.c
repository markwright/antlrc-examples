...
int processFile(ProgramOptions *opts, const char *fileName)
{
  ...
  pArithLexer lexer = ArithLexerNew(input);
  if (lexer == (pArithLexer)0)
  {
  ...
  pArithParser parser = ArithParserNew(tstream);
  if (parser == (pArithParser)0)
  {
    printf("ERROR: ArithParserNew failed\n");
...
main (int argc, const char *argv[])
{
  ...
  /* hs_add_root(__stginit_ArithInterface); */
